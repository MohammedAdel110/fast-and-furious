...
void timer(int) {
    glutPostRedisplay();
    glutTimerFunc(1000 / (FPS * 80), timer, 0); // More frequent updates at high speed
    if (gv == 1 && !pause) {
        if (FPS < 1) FPS += 0.01;
        score += FPS;

        float speedMultiplier = 30.0f;
        // Smaller increments but more frequent
...
void processKeys(unsigned char key, int x, int y) {
    ...
    case 'L':
        ...
        if (radioOn) {
            ...
            switch (channelNum) {
            case 1:
                PlaySound(TEXT("C:\\Users\\moham\\Downloads\\quran.wav"), NULL, SND_FILENAME | SND_ASYNC);
                break;
            case 2:
                PlaySound(TEXT("C:\\Users\\moham\\Downloads\\Car Horn Sound Effect _ Car Horn Traffic Sound Effect.wav"), NULL, SND_FILENAME | SND_ASYNC);
                break;
            case 3:
                PlaySound(TEXT("C:\\Users\\moham\\Downloads\\videoplayback.wav"), NULL, SND_FILENAME | SND_ASYNC);
                break;
            case 4:
                break;
            case 5:
...
void sky() {
    glDisable(GL_LIGHTING); // Sky shouldn't be affected by lighting
...
void score_board() {
    ...
    if (gv == 0) {
        ...
        // خلفية مستطيلة للسكور
        float padding = 0.5;
        float x1 = xMin - padding;
        float y1 = yMax - yMax / 7 - padding;
        float x2 = xMin + 3; // حسب حجم النص
        float y2 = yMax - yMax / 25 + padding;
        drawRectangle(x1, y1, x2, y2, zMax - 1.1, 0, 0, 1); // خلفية داكنة
        ...
        // مستطيل خلفي لتجميع بيانات النهاية
        float x1 = xMin - 1;
        float y1 = yMax - yMax / 25 - 1.5;
        float x2 = xMin + 3;
        float y2 = yMax - yMax / 2.5 + 2;
        drawRectangle(x1, y1, x2, y2, zMax - 1.1, 0.2, 0.2, 0.2); // خلفية داكنة
...
void collision() {
    // Define collision threshold (how close cars need to be to collide)
    const float collisionThreshold = 3.0f; // Adjust this value as needed

    //KIll check car - now checks if opponent car is within threshold distance
...
void load(int imgnum) {
    if (imgnum == 1) {
        data = stbi_load("4.jpg", &width, &height, &nrChannels, 0);
        check(data);
    }
    else if (imgnum == 2) {
        data = stbi_load("sky.jpeg", &width, &height, &nrChannels, 0);
        check(data);
    }
}
